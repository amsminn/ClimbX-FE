...
class VideoApi {
  ...
  static Future<List<Video>> getCurrentUserVideos() async {
    try {
      ...
      developer.log('사용자 영상 목록 조회 - nickname: $finalNickname', name: 'VideoApi');
  ...
  static Future<VideoUploadResponse> requestUploadUrl({
    ...
    try {
      ...
      developer.log('presigned URL 요청 성공 - videoId: ${uploadResponse.videoId}', name: 'VideoApi');
        ...
        onSendProgress: onProgress != null 
            ? (sent, total) {
                ...
                if (progress - lastLoggedProgress >= 0.05 || progress >= 1.0) {
                  developer.log('업로드 진행률: ${(progress * 100).toStringAsFixed(1)}%', name: 'VideoApi');
  ...
  static Future<String> uploadVideo({
    ...
    try {
      ...
      developer.log(
        '영상 업로드 시작 - 파일: ${file.path}, 크기: ${fileSize}bytes, 확장자: $fileExtension',
        name: 'VideoApi'
      );
      ...
      if (fileSize > maxSize) {
        throw Exception('파일 크기가 너무 큽니다. (최대: ${maxSize ~/ (1024 * 1024)}MB)');
      }

      // 3. 지원 형식 검증
      const supportedFormats = ['mp4', 'mov', 'avi'];
      if (!supportedFormats.contains(fileExtension)) {
        throw Exception('지원하지 않는 파일 형식입니다. (지원 형식: ${supportedFormats.join(', ')})');
      }
      ...
      developer.log('전체 업로드 프로세스 완료 - videoId: ${uploadResponse.videoId}', name: 'VideoApi');
